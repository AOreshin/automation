...
public final class StepRepositoryAspect {
    private final Set<Class<?>> STEP_REPOSITORY_INTERFACES = new AnnotatedTypeScanner(true,
    ...
    public Object processPageObjectMethod(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Set<Class<?>> stepRepositoryInterfaces = getImplementedStepRepositoryInterfaces(implementedInterfaces);
    ...
    private Object findAndProcessMethod(ProceedingJoinPoint joinPoint, MethodSignature methodSignature,
                                        Set<Class<?>> stepRepositoryInterfaces) throws Throwable {
    ...
    private void stopBrokenStep(Throwable throwable) {
        getLifecycle()
                .updateStep(s -> s.setStatus(getStatus(throwable).orElse(Status.BROKEN)).setStatusDetails(getStatusDetails(throwable).orElse(null)));
        getLifecycle()
                .stopStep();
    }
    ...
    private Method searchForMethodInImplementedStepRepositories(Set<Class<?>> stepRepositoryInterfaces,
                                                                MethodSignature signature) {
        return stepRepositoryInterfaces
                .stream()
