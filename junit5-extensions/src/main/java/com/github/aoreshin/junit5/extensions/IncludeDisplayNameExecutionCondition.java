...
public final class IncludeDisplayNameExecutionCondition implements ExecutionCondition {
    ...
    public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
        if (DISPLAY_NAMES_LIST.size() != 0) {
            ...
            if (isContained) {
                return enabled(displayName + " is present in " + PROPERTY_NAME + " property " + DISPLAY_NAMES_LIST);
            }

            return disabled(displayName + " is not present in " + PROPERTY_NAME + " property " + DISPLAY_NAMES_LIST);
        }
