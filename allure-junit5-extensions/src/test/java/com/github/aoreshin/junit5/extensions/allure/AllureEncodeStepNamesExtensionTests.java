...
final class AllureEncodeStepNamesExtensionTests {
    ...
    void afterEachTest() {
        ...
        List<String> shouldNotBePresent = List.of(
                "123456",
                "qwerty",
                "impossibleToCrack");
        ...
        List<Executable> executables = processedStepResults
                .stream()
                .map(stepResult -> (Executable) () -> {
                    String stepName = stepResult.getName();
                    assertFalse(containsUnexpectedValue(shouldNotBePresent, stepName),
