...
final class WebDriverPageObjectFactory {
    ...
    <T extends WebDriverPageObject<T>> T createPageObject(final Class<T> pageObjectClass) {
        ...
        WebDriverPageObject<? extends WebDriverPageObject<?>> cachedPageObject = pageObjectCache.get(pageObjectClass);
    ...
    private <T extends WebDriverPageObject<T>> void isSupported(final Class<T> pageObjectClass) {
        LOGGER.debug("Validity check PageObject " + pageObjectClass.getName() + " and PageObjectFactory " +
                this.getClass().getName());
        Class<?> superClass = pageObjectClass.getSuperclass();

        if (!superClass.equals(WebDriverPageObject.class)) {
            throw new PageNotInitializedException(this.getClass().getName() + " does not support PageObject " +
    ...
    private <T extends WebDriverPageObject<T>> void initPage(final T pageObjectInstance, Class<T> pageObjectClass) {
        //Страница уже проверена на валидность, можно спокойно кастовать
        @SuppressWarnings("unchecked")
        Class<WebDriverPageObject<?>> superClass = (Class<WebDriverPageObject<?>>) pageObjectClass.getSuperclass();

        Map<String, Object> nameAndValues = Map.of(
                "driver", webDriver,
